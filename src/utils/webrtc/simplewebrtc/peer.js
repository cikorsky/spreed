 ...
 *
 * Currently remote remote video can be blocked only when the HPB is used, so
 * this method should be called immediately before creating the answer (the
 * answer must be created in the same "tick" that this method is called).
 *
...
Peer.prototype.setRemoteVideoBlocked = function(remoteVideoBlocked) {
	...
	if (!this.receiverOnly || !this.parent.config.connection.hasFeature('subscriber-update')) {
		return
	}
